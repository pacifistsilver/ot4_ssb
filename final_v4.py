...
metadata = {
    ...
    'author': 'OT4: Daniel Luo, Lihao Tao, Agnes Cheung',
    'apiLevel': '2.14'
}
...
# 2x tip boxes would there be enough for 1x destination plate scenario -- each destination plate setup takes around 1.5x tip boxes
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    p300_tip_3 = protocol.load_labware(LABWARE[0], 7) # use this for more than 1 replicate 
    p300_multi = protocol.load_instrument('p300_multi_gen2', 'left', tip_racks=[p300_tip_1, p300_tip_2]) # add corresponding tip boxes here
    ...
    dest_list = [dest_1][:replicates] # please add the detination plates names here
    ...
    for col in list(range(0, 8)) + list(range(9, 11)): # select col 1-8 and col 10-12 in the source plate
        dest = source.columns()[col]
        if pbs_well_volume > 1200: # if the aspirated volume passes certain amount, move to the next pbs source column to avoid aspirating nothing 
            pbs_well_volume: int = 0 
    ...
    p300_multi.mix(3, FLOW_VOL["mix_vol"], source.columns()[7][0]) # setup the highest concentration for inducer A at col 8 in the source plate
    p300_multi.blow_out(source.columns()[7][0])
    for col in range(7, 0, -1): # steps for serial dilution starting from col 8 to col 1 in the source plate -- fixed rows, moving col by col
    ...
    p300_multi.aspirate(FLOW_VOL["asp_vol"], source.columns()[0][0]) # remove 300 ul from col 1
    p300_multi.drop_tip()
    protocol.comment('Inducer A serial dilution complete')
 

    # 3. inducer B serial dilution
    inducerb_col = 11 # setup the column for inducer B to implement the serial dilution

    for replicate in range(0, replicates): # the number of cols of inducer B serial dilution will be depends on the number of replicates
        p300_single.pick_up_tip()
        p300_single.aspirate(FLOW_VOL["asp_vol"], reservoir.columns()[2][0], rate = 0.6) # a slow aspiration rate for viscosity 
        ...
        for row in range(7, 0, -1): # steps for inducer B serial dilution -- fixed col, moving row by row
    ...
    protocol.comment(f'''Inducer A added to plate col 2 (only-A) and col 4 (positive control))''')
    
    # 3. add stock B to destination plate col 3 (only-B) and col 4 (positive control)
    p300_multi.pick_up_tip()
    dest1_col_B_only = dest_1.wells_by_name()['A3']
    # please comment in the dest2 if more replicates are designed
    ...
    protocol.comment(f'''Inducer B added to plate col 3 (only-B) and col 4 (positive control)''')

    # destination plate setup -- repeat for the number of replicates 
    for plate_idx, dest in enumerate(dest_list, start=1):
        ...
        for offset in range(8):            
            source_col_index = 7 - offset   # col 8-1
            dest_col_index = 11 - offset    # col 12-5
        ...
        for offset in range(2):            
            ...
            p300_multi.blow_out(dest_B_well.top()) # dispense and blow out both at top because no tip change here (do not want to mess with existed A in each well)
        ...
        # 6. Add cells (PBS mimic) to make a 100 ul system for each well
        cell_dict_1 = {
            dest_col_PBS: 70,
            dest_col_A_only: 70,
            dest_col_B_only: 70,
        ...
        # add 40 ul PBS to destination plate col 5–12 (col 4–11)
        for i in range(4, 12):
            cell_well = dest.columns()[i][0]
            cell_dict_2[cell_well] = 40

        # aspirate and dispense PBS to each well with designed volumn to make up a 100 ul system for each well
