...
## 2 replicates + original run
# 1x source plate, 1x reservoir plate, 3x destination plates? 3x tip boxes
# would there be enough volume in the source and reservor? 
# how many tips would be end up using? i.e. how many tip boxes would we need
#
...
# hnadle multiple replicates and adjust plate count and position > map replicate count to labware definitions?
# load config data
"""
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    PBS_source = reservoir.wells_by_name()['A6']   # col 1
    PBS_source_array = [reservoir.columns()[6], reservoir.columns()[7]]
    ...
    p300_multi.pick_up_tip()
    p300_multi.aspirate(FLOW_VOL["asp_vol"], reservoir.columns()[1][0], rate = 0.6)
    ...
    for col in range(7, 0, -1):
        ...
        p300_multi.dispense(FLOW_VOL["disp_vol"], dest, rate = 0.4)
    ...
    # 1. add PBS to destination plate col 1
    p300_multi.pick_up_tip()
    ...
    for plate_idx, dest in enumerate(dest_list, start=1):
        ...
        B_source_col_idx = 10 + (plate_idx - 1)  # 9,10,11
